  ...
  return (
    <div className="bg-black/25">
      <div className="bg-gray-50 p-10 flex-col">
        ...
        {mission.map((item) => (
          <div className="flex flex-col md:flex-row bg-white p-4 items-center rounded-md shadow-md cursor-default" key={item.mission_id}>
            ...
            <div className="flex items-center h-40 w-40 justify-center bg-slate-200 p-4">
              <p className={`${item.active ? 'bg-blue-500 text-white rounded-sm p-1' : 'bg-slate-400 text-white rounded-sm p-1'} `}>
                {item.active ? 'Active Member' : 'Not a Member' }
              </p>
            </div>
